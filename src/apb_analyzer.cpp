    ...
    if (snapshot.psel && !snapshot.psel_has_x && !snapshot.penable) {
        ...
        if (!is_write) {
            if (m_pending_writes.count(paddr)) {
                m_statistics.record_read_write_overlap_error(
                    {snapshot.timestamp_ps, paddr});
            }
        } else {
            m_pending_writes[paddr] = snapshot.timestamp_ps;
        }
        ...
        if (m_current_transaction.is_write) {
            m_current_transaction.pwdata_val = snapshot.pwdata;  // 寫操作的數據
...
void ApbAnalyzer::handle_access_state(const SignalState& snapshot) {
    ...
    if (!snapshot.psel || snapshot.psel_has_x || !snapshot.penable || snapshot.penable_has_x) {
        if (m_current_transaction.is_write) {
            m_pending_writes.erase(m_current_transaction.paddr);  // 清除寫操作的待處理項
        }
        m_current_transaction.reset();
        m_current_apb_fsm_state = ApbFsmState::IDLE;
        return;
